...
var euclidean_score  = function(dataset,p1,p2){
    ...
    var existp1p2 = {};//store item existing in both item//if dataset is in p1 and p2 
    //store it in as one
    for(var key in dataset[p1]){
        if(key in dataset[p2]){
            existp1p2[key] = 1
        }if(len(existp1p2) ==0) return 0;//check if it has a datavar sum_of_euclidean_dist = [];//store the  euclidean distance
...
var pearson_correlation = function(dataset,p1,p2){
        ...
        if(num_existence ==0) return 0;//store the sum and the square sum of both p1 and p2
        ...
        var st2 = p2_sq_sum -Math.pow(p2_sum,2)/num_existence;var denominator = Math.sqrt(st1*st2);if(denominator ==0) return 0;
        else {
